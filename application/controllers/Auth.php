...
class Auth extends CI_Controller
{
    ...
    public function forgot_password()
    {
            ...
            if(!empty(trim($this->input->post('identity')))){
                $info = $this->db->get_where('users', array('email'=>$this->input->post('identity')))->row();
                if($info){
                    $username = $info->username;
                }else{
                    $this->session->set_flashdata('message', 'User email does not exist');
                }

            }

            if(trim($this->input->post('identity_number'))<>''){
                $regnoInfo = $this->db->get_where('students', array('registration_number'=>$this->input->post('identity_number')))->row();
                if($regnoInfo){
                    $username = $this->db->get_where('users', array('id'=>$regnoInfo->user_id))->row()->username;
                }else{
                    $this->session->set_flashdata('message', 'No information found for this Registration Number');
