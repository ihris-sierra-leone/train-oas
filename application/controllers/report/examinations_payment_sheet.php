...
$sheet->setTitle("EXAMINATIONS FEE");
if (!is_null($year)) {

    $sheet->setCellValue('B3', 'EXAMINATIONS PAYMENT FOR THE YEAR : ' . $year);

} else if (!is_null($from) && !is_null($to)) {
//$sheet->setCellValue('B3','MEMBER LIST '.$yea.' -  2 ');
    $sheet->setCellValue('B3', 'EXAMINATIONS PAYMENT FROM ' . $from_date . ' TO ' . $to_date);
} else {
    $sheet->setCellValue('B3', ' EXAMINATIONS FEE LIST ');
}
...
if (isset($year) && $year != '') {
    $where .= " AND YEAR(a.createdon) = '$year' ";
}
if (isset($from_date) && $from_date != '') {
    $where .= " AND DATE(a.createdon)>='" . format_date($from_date) . "' ";
}
if (isset($to_date) && $to_date != '') {
    $where .= " AND DATE(a.createdon)<='" . format_date($to_date) . "' ";
}

//exit($where);
$applicant = $this->db->query("SELECT a.*,app.FirstName, app.MiddleName,app.LastName,app.user_id FROM examinations_payment as a LEFT JOIN application as app ON (a.user_id=app.user_id) $where  ORDER BY a.createdon DESC ")->result();

//$applicant = $this->db->query("SELECT p.*, a.first_name, a.other_names, a.surname FROM  students as a INNER JOIN examinations_payment as p ON (p.user_id=a.user_id) ")->result();
...
$application_type = "examinations payment list";
