...
$sheet->setTitle("ANNUAL SUBSCRIPTION FEES");
if (!is_null($year)) {

    $sheet->setCellValue('B3', 'ANNUAL SUBSCRIPTION FEES FOR : ' . $year);

} else if (!is_null($from) && !is_null($to)) {
//$sheet->setCellValue('B3','MEMBER LIST '.$yea.' -  2 ');
    $sheet->setCellValue('B3', 'ANNUAL SUBSCRIPTION FROM ' . $from_date . ' TO ' . $to_date);
} else {
    $sheet->setCellValue('B3', ' ANNUAL SUBSCRIPTION FEES ');
}
...
//exit($where);
$applicant = $this->db->query("SELECT a.*,app.FirstName, app.MiddleName,app.LastName FROM annual_fees as a LEFT JOIN application as app ON (a.user_id=app.user_id) $where ORDER BY a.createdon DESC  ")->result();
...
$application_type = "annual payment list";
