...
class Admin extends CI_Controller
{
    ...
    function create_user($id = null)
    {
        ...
        if (is_null($id)) {
           ...
           // $this->form_validation->set_rules('email', 'Email', 'required|valid_email|is_unique[users.email]');
            ...
            $this->form_validation->set_rules('email', 'Email', 'required' . $id . ']');
            ...
            if (is_null($id)) {
                ...
                if ($user_id) {
                    ...
                    if ($group_id == 15) {
                        ...
                        $this->academic_model->add_department(array('head' => $user_id), $access_area_id);

                    }

                    //Send Email in backhround
                    execInBackground("response new_account_created " . $user_id . ' ' . $password);
                    execInBackground("response send_user_credential " . $user_id . ' ' . $password.' '.$current_user->id);
                ...
                if ($user_id) {
                    ...
                    //User is principal
                    if ($group_id == 100) {
                        $this->academic_model->add_schools(array('principal' => $id), $access_area_id);
                    }else if($group_id == 3){
                        $this->academic_model->add_department(array('head' => $id), $access_area_id);
                    }
    ...
    function user_list()
    {
        ...
        $where = " WHERE u.campus_id = $current_campus->id AND u.user_status <> 1 ";
...
function manage_database(){
    ...
    public function del_user($id){
        ...
        $this->data['bscrum'][] = array('link' => 'user_list/', 'title' => 'User List');

        $this->ion_auth_model->mv_user($id);
        $this->data['active_menu'] = 'manage_users';
        $this->session->set_flashdata('message', show_alert('One user record deleted','success'));
        redirect(site_url().'/user_list');
