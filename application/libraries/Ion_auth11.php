	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			...
			if ($user)
			{
				...
				$current_user = $user;
				$notify = array();
				$notify['current_user'] = $current_user;
				$notify['data'] = $data;
				$email_message = $this->load->view('email_template/forgot_password',$notify,true);
				$send = $this->mail->send_email('PASSWORD RESET - SIMS NIT', $email_message, array($current_user->email));
